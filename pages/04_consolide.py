...
def consolider(df_src_json, wk_cols):
    ...
    if 'ORIGINE' in df.columns and 'PROGRAMME' in df.columns:
        def calc_origine(grp):
            origines = set(grp['ORIGINE'].dropna().unique()) - {''}
            if len(origines) == 1:
                orig = list(origines)[0]
                if orig == 'LPC':
                    # Afficher les noms de programmes distincts
                    progs = grp['PROGRAMME'].dropna().astype(str)
                    progs = progs[progs.str.strip() != ''].unique()
                    return ', '.join(sorted(progs)) if len(progs) > 0 else 'LPC'
                return orig  # CARNET ou PROJET
            return ''  # plusieurs sources mélangées
        origine_map = df.groupby('REF_ARTICLE_SERTA').apply(calc_origine)
    else:
        origine_map = None
