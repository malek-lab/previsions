...
def reequilibrer_semaines_avance_retard(df, wk_cols_apres_cutoff, df_hist,
    ...
    - df_hist : sortie de get_historique_ventes() -- CLIENT_CODE, ITEM_CODE,
           DATE_FACTURE, QTE. Si vide/None, retourne df inchangé.
    ...
    dfh = df_hist.copy()
    dfh['CLIENT_CODE'] = dfh['CLIENT_CODE'].astype(str).str.strip()
    dfh['ITEM_CODE']   = dfh['ITEM_CODE'].astype(str).str.strip()
    ...
    facture_par_semaine = (
        dfh.groupby(['CLIENT_CODE', 'ITEM_CODE', 'SEMAINE_FACTURE'])['QTE']
        .sum().to_dict()
    )
