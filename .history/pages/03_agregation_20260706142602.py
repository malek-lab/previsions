...
def charger_carnet(couples_lpc, date_du, date_au):
    ...
    # CORRIGE : bug trouve par diagnostic -- comparer wk_label_to_date(semaine)
    # (le LUNDI de la semaine ISO) a date_du/date_au introduit un decalage
    # pouvant aller jusqu'a 6 jours pres des frontieres de semaine (ex: cutoff
    # au 30/06/2026 qui tombe un mardi -> le lundi de sa semaine est le 29/06,
    # donc une commande ARC au 01/07 se voyait exclue a tort puisque
    # 29/06 < 30/06). On filtre desormais sur la VRAIE date ARC
    # (CLIENT_ACK_DATE), pas sur le lundi de la semaine qui la contient.
    df = df[(df['CLIENT_ACK_DATE'].dt.date >= date_du) & (df['CLIENT_ACK_DATE'].dt.date <= date_au)]
    if df.empty:
        return pd.DataFrame()
