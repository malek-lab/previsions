...
# AJOUT : aller-retour telecharger / reimporter -- permet d'editer/completer
# une colonne a la main dans Excel (ex: renseigner MONTH_X_QTY manquant) puis
# de reinjecter le fichier corrige, qui remplace df_proj avant la suite du
# traitement (separation nouveaux/existants, integration).
st.markdown("### 📤 Éditer hors-ligne (optionnel)")
...
with col_dl:
    st.download_button(
        ...
        help="Édite/complète une colonne dans Excel (ex: une quantité manquante), "
             "puis réimporte le fichier ci-contre pour qu'il remplace ce tableau."
    )
with col_ul:
    ...
    if fichier_reimporte is not None:
        try:
            ...
            # Re-forcer les types comme a la sortie normale de ventiler_vers_semaines()
            for col in ['NUM_PROJET', 'REF_ARTICLE_SERTA', 'CODE_SELECTION', 'UP_PRINCIPALE',
                        'BUSINESS_NAME', 'CODE_CLIENT', 'SERTA_SO_CLIENT_NAME',
                        'SERTA_SO_CLIENT_GROUP_NAME', 'PROJECT_MANAGER', 'SALES_PERSON',
                        'STATUT', 'ORIGINE']:
            ...
            df_proj = df_reimporte
            st.session_state['df_projets_raw'] = df_proj
            st.success(f"✅ Fichier réimporté — {len(df_proj)} lignes utilisées à la place du chargement initial.")
        except Exception as e:
...
if st.button("✅ Intégrer dans la consolidée (page 04)", type="primary",
    ...
    with st.expander("📝 Journal — références précises intégrées à cette étape"):
        if _refs_nouveaux_liste:
            st.markdown(f"**Nouvelles ({len(_refs_nouveaux_liste)})** : " + ", ".join(_refs_nouveaux_liste))
        if _refs_doublons_liste:
            st.markdown(f"**Doublons gardés ({len(_refs_doublons_liste)})** : " + ", ".join(_refs_doublons_liste))
        if not _refs_nouveaux_liste and not _refs_doublons_liste:
