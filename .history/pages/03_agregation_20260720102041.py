...
from shared import get_engine, logo_sidebar, wk_cols_from_df, to_excel_bytes, nettoyer_code, nettoyer_codes_dataframe, get_historique_ventes, wk_label_to_date, reequilibrer_semaines_avance_retard
    ...
    if 'df_consolide' not in st.session_state:
        ...
        if 'CODE_CLIENT' not in df_c.columns or 'REF_ARTICLE_SERTA' not in df_c.columns:
            ...
            if 'df_synthese' in st.session_state:
                ...
                # AJOUT : rééquilibrage avance/retard semaine par semaine, APRES
                # le cutoff uniquement. Si le facturé réel d'une semaine dépasse
                # le prévu programme de cette même semaine (livraison en avance),
                # l'excédent est retiré de la semaine SUIVANTE (cascade), pour ne
                # pas compter deux fois la même demande.
                #
                # IMPORTANT : ceci est un tableau d'affichage SEPARE -- il ne
                # touche JAMAIS a Qty_YTD ni Qty {annee}, deja valides a 100%
                # (voir RÈGLE plus haut). Le total annuel reste inchangé.
                st.markdown("---")
                st.subheader("⚖️ Semaines après cutoff — rééquilibrage avance/retard")
                st.caption(
                    "Si le facturé d'une semaine dépasse le prévu programme, l'excédent "
                    "est déduit de la semaine suivante (cascade) — vue d'affichage "
                    "uniquement, ne modifie pas Qty_YTD ni Qty {a} (déjà validés).".format(a=annee_actuelle)
                )

                def _wk_label_to_date(label):
                    return wk_label_to_date(label)

                # CORRIGE : wk_n recalcule localement (pas de dependance a une
                # variable externe qui peut ne pas etre definie si le bloc qui
                # la calculait plus haut n'a pas ete execute cette fois --
                # cause du NameError constate en production). df_c et
                # annee_actuelle restent necessaires et doivent etre definis
                # avant ce bloc.
                wk_tous_local = wk_cols_from_df(df_c)
                wk_n = [c for c in wk_tous_local if annee_semaine(c)[0] == annee_actuelle]

                wk_apres_cutoff = sorted(
                    [w for w in wk_n if (_wk_label_to_date(w) or dt.date(1900, 1, 1)) >= date_cutoff],
                    key=lambda w: _wk_label_to_date(w) or dt.date(1900, 1, 1)
                )

                if not wk_apres_cutoff or 'df_hist_detail' not in st.session_state:
                    st.info("Aucune semaine après le cutoff à afficher, ou historique de facturation indisponible.")
                else:
                    df_hist_bal = st.session_state['df_hist_detail']
                    prevu_wk = (df_c.groupby(['CODE_CLIENT', 'REF_ARTICLE_SERTA'])[wk_apres_cutoff]
                                .sum().reset_index())
                    nb_avant = prevu_wk[wk_apres_cutoff].sum().sum()
                    ajuste = reequilibrer_semaines_avance_retard(
                        prevu_wk, wk_apres_cutoff, df_hist_bal,
                        col_client='CODE_CLIENT', col_ref='REF_ARTICLE_SERTA',
                        col_client_hist='CODE_CLIENT', col_ref_hist='REF_ARTICLE_SERTA')
                    nb_apres = ajuste[wk_apres_cutoff].sum().sum()

                    st.caption(f"Semaines traitées : {wk_apres_cutoff[0]} → {wk_apres_cutoff[-1]} "
                               f"({len(wk_apres_cutoff)} semaines). Total avant/après réajustement "
                               f"(normal que ça diffère, c'est un déplacement, pas une perte) : "
                               f"{nb_avant:,.0f} → {nb_apres:,.0f}")
                    st.dataframe(ajuste, width='stretch', height=400)
                    st.download_button(
                        "📥 Télécharger le tableau rééquilibré (Excel)",
                        data=to_excel_bytes(ajuste),
                        file_name=f"semaines_reequilibrees_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                        key="dl_reequilibre"
                    )
