...
# ============================================================================
# AJOUT : persistance des versions de carnet en base de donnees, pour
# survivre aux rechargements de page / fermetures de session / mises a jour
# de l'app -- st.session_state seul ne suffit pas car il est perdu des que le
# navigateur/l'app redemarre. Table dediee CARNET_SNAPSHOTS (creee au premier
# usage si absente). Le DataFrame est serialise en JSON compresse gzip et
# stocke en VARBINARY(MAX) -- evite une dependance a pyarrow/parquet si non
# deja installe, taille raisonnable pour ce volume de lignes (~1000-1500).
#
# NOTE : les vues source (V_EXPEDITION_SUIVI, V_SUPPLY_CHAIN) vivent dans
# [master].[dbo] d'apres le reste du fichier -- la table de snapshots est
# creee dans le meme schema par coherence, mais stocker des tables
# applicatives dans "master" n'est pas idealement recommande (c'est la base
# systeme de SQL Server). Si une autre base est disponible pour l'app,
# remplacer [master].[dbo].[CARNET_SNAPSHOTS] par [NomAppDB].[dbo].[...].
# ============================================================================
...
with st.sidebar:
    ...
    date_carnet_du = st.date_input("📅 Semaines à partir du",
        ...
        # rechargement de la page, puisque value=date_prevision_ref est
        # recalcule (et peut changer, ex: base sur "aujourd'hui") a chaque
        # execution du script. Avec une cle explicite, Streamlit memorise la
        # derniere valeur choisie par l'utilisateur et ne la reecrase plus.
    ...
    st.markdown("---")
    # AJOUT : nom optionnel de la version -- permet de reconnaitre facilement
    # un snapshot dans le selecteur plus bas (ex: "S29 court terme" vs
    # "Vision annuelle"), plutot que de se fier uniquement aux dates.
...
# AJOUT : REF_ARTICLE_SERTA n'etait JAMAIS nettoyee ici (contrairement a
# CODE_CLIENT juste au-dessus) -- si le fichier source avait un espace ou un
# ".0" parasite sur une ref, couples_lpc ne matchait jamais avec le carnet,
# et l'exclusion anti-doublon ne retirait donc jamais rien (0 ligne exclue,
# meme volume carnet que Supply Chain brut -- symptome observe et corrige ici).
...
def charger_groupe_article(refs_tuple):
    ...
    for i, lot in enumerate(lots):
        # CORRIGE : chaque element doit etre enveloppe individuellement par
        # '' ... '' (echappement correct pour OPENQUERY), pas juste au debut
        # et a la fin de la liste jointe -- l'ancienne version produisait
        # IN (''ref1', 'ref2', ...''), avec un guillemet manquant a chaque
        # frontiere entre elements, d'ou l'erreur de syntaxe SQL.
...
# RETIRE (episode 2) : le filtre PDR/composants sur le programme LPC a ete
# retire -- preuve trouvee par comparaison avec le fichier de reference
# externe : 9 refs PDR/composants (dont une contenant litteralement "PDR"
# dans son nom, DU109013PDR) sont classees "PREV CLIENT" (vraie prevision)
# cote reference, pas exclues. Sophie semble donc garder les refs PDR dans
# sa vue PREVISION/programme, et ne les exclure QUE de sa vue ENCOURS/carnet.
# Le filtre PDR reste actif uniquement cote carnet (V_SUPPLY_CHAIN +
# charger_retard_reconstruit_historique), pas ici.
#
# GROUPES_HORS_PERIMETRE = ['PDR', 'PFPDR', 'MAUNIT', 'MAJOIN']
#
...
#                 f"(Pièces De Rechange/composants, hors périmètre suivi par Sophie).")
...
def charger_suivi_carnet(date_prevision, date_ventilation):
    """
    CORRIGE (bug majeur trouve par diagnostic manuel le 21/07) : la vue
    [V_EXPEDITION_SUIVI] est definie avec un filtre WHERE STATUT < 7 (voir
    definition de la vue -- ne garde que les expeditions PAS ENCORE
    facturees). Consequence : au moment ou une ligne EST facturee
    (DATE_FACTURE se remplit), son STATUT passe a >= 7 et elle sort
    IMMEDIATEMENT de la vue. DATE_FACTURE ne peut donc structurellement
    JAMAIS etre non-NULL dans [V_EXPEDITION_SUIVI], quelle que soit la
    periode interrogee -- QTE_FACTUREE_ENCOURS valait donc TOUJOURS 0,
    silencieusement, depuis le debut.

    Diagnostic (21/07/2026) : test direct sur DW.VENTE.V_EXPEDITION (source
    brute, sans le filtre STATUT<7) via le linked server [SRV-MSSQLDB] a
    confirme la presence de vraies factures recentes (20849 unites sur 328
    couples CODE_CLIENT/REF_ARTICLE_SERTA, derniere facture le jour meme).

    FIX : QTE_EN_TRANSITE_RETARD / QTE_EN_TRANSITE_ENCOURS restent calcules
    depuis [V_EXPEDITION_SUIVI] (corrects : ces deux colonnes ne dependent
    QUE de DATE_FACTURE IS NULL, qui est toujours vrai dans cette vue -- pas
    de probleme ici). QTE_FACTUREE_ENCOURS, lui, est desormais calcule a
    part depuis DW.VENTE.V_EXPEDITION en direct (sans filtre STATUT), via
    OPENQUERY sur le linked server, puis fusionne aux deux premieres
    colonnes sur CODE_CLIENT + REF_ARTICLE_SERTA.
    """
    from sqlalchemy import text
    engine = get_engine()
    if engine is None:
