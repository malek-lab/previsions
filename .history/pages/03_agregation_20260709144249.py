            ...
            df = pd.read_sql(text(f"""
                ...
                FROM OPENQUERY([SRV-MSSQLDB], '
                    ...
                    HAVING (MAX(VEXP.DATE_FACTURE) > ''{cutoff_str}''
                        ...
                        -- CORRIGE : QTE_COMMANDEE (quantite commandee a
                        -- l'origine, ne change jamais) au lieu de
                        -- QTE_COMMANDEE-QTE_LIVREE (reste a livrer
                        -- AUJOURD'HUI). Ce reste est mecaniquement a 0 pour
                        -- toute commande deja livree depuis le cutoff -- donc
                        -- l'ancien filtre excluait exactement les commandes
                        -- qu'on cherche a recuperer (closes depuis, mais
                        -- ouvertes au cutoff), pas seulement le vrai bruit.
